...
bl_info = {
    "name": "Scour/Quest Tools",
    "author": "<savory",
    "version": (0, 1),
    "blender": (3, 60, 0),
    "description": "various unreal compatible tools for quest/scour development",
    "category": "Game Dev",
}
...
class ScriptRunner:
    TARGET_SCRIPT_NAME: str = "tools_main"
    TARGET_SCRIPT_PATH: str = str(pathlib.Path(__file__).parent.absolute() / f"{TARGET_SCRIPT_NAME}.py")
    TARGET_SCRIPT_FUNCTION: str | None = "register"  # this can be None if you just want to run the whole script
...
class UnrealEditorPath(bpy.types.AddonPreferences):
    bl_idname = __name__

    unreal_editor_path: bpy.props.StringProperty(subtype="FILE_PATH",
        name="Unreal Editor Filepath",
        description="Select your unreal editor filepath, (Engine/)",
    )

    def draw(self, context):
        layout = self.layout
        layout.prop(self, "unreal_editor_path")


def register() -> None:

    bpy.utils.register_class(UnrealEditorPath)
...
def unregister() -> None:
    ...
    bpy.utils.unregister_class(UnrealEditorPath)
